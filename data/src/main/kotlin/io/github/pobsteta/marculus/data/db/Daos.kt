...
interface TigeDao {
    ...
    @Query(
        "UPDATE tige SET essence = :essence, classe = :classe, action = :action, " +
            "quantite = :quantite, hauteurTexte = :hauteur, qualiteArbre = :qualite, modifie = :modifie WHERE uuid = :uuid",
    )
    ...
    @Query(
        "UPDATE tige SET latitude = :lat, longitude = :lon, parcelle = :parcelle, " +
            "qualiteFix = :qualiteFix, precisionM = :precisionM, modifie = :modifie WHERE uuid = :uuid",
    )
    suspend fun majPosition(
        uuid: String,
        lat: Double?,
        lon: Double?,
        parcelle: String?,
